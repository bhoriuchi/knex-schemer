...
module.exports = function(config) {
	...
	function ignorable(col, colName) {
		...
		var ignore  = (!col.hasOwnProperty(OPTS.ignore) || !col.ignore) ? false: true;
		var dashCol = (colName !== null && colName.substring(0,1) === '_') ? true: false;
		
		
		// check that there are no ignorable conditions
		if (!ignore && !related && !extend && !dashCol) {
			return false;
		}
		
		
		// if there are any ignore conditions the column is ignorable
		return true;
	}
	...
	function required(col, colName) {
		...
		if (!nullable(col) && !ignorable(col, colName)) {
			return true;
		}
		return false;
	}
	...
	function checkSchema(data, tableSchema) {
		...
		for(var i = 0; i < data.length; i++) {
			...
			for(var j = 0; j < cols.length; j++) {
				...
				if (!data[i].hasOwnProperty(colName) && required(col, colName)) {
					return false;
				}
			}
		}
