				...
				.then(function(result) {
					...
					if (result.length > 0 && result[0].length > 0) {
						...
						_.forEach(result[0], function(column) {
							...
							info[name].unsigned     = _.contains(column.COLUMN_TYPE.split(' '), 'unsigned');
							info[name].primary      = column.COLUMN_KEY ? _.contains(column.COLUMN_KEY.split(','), 'PRI') : false;
							info[name].unique       = column.COLUMN_KEY ? _.contains(column.COLUMN_KEY.split(','), 'UNI') : false;
						});
						return info;
					}
				});
				...
				.then(function(result) {
					...
					if (_.has(result, 'rows') && result.rows.length > 0) {
						...
						_.forEach(result.rows, function(column) {
							...
							info[name].unique       = null; //column.COLUMN_KEY ? _.contains(column.COLUMN_KEY.split(','), 'UNI') : false;
